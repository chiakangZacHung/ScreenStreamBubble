        ...
        override fun onImageAvailable(reader: ImageReader) {
            synchronized(this@BitmapCapture) {
                ...
                try {
                    ...
                    // Save frame every 15 seconds
                    if (now - lastSaveTimeMillis >= 15000) {
                        lastSaveTimeMillis = now
                        try {
                            val timestamp = SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(Date())
                            val filename = "ScreenStream_$timestamp.jpg"

                            val contentValues = ContentValues().apply {
                                put(MediaStore.MediaColumns.DISPLAY_NAME, filename)
                                put(MediaStore.MediaColumns.MIME_TYPE, "image/jpeg")
                                put(MediaStore.MediaColumns.RELATIVE_PATH, "Pictures/ScreenStream")
                            }

                            val uri = serviceContext.contentResolver.insert(
                                MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
                                contentValues
                            )

                            uri?.let { imageUri ->
                                serviceContext.contentResolver.openOutputStream(imageUri)?.use { out ->
                                    bitmap.compress(Bitmap.CompressFormat.JPEG, mjpegSettings.data.value.jpegQuality, out)
                                    out.flush()
                                }

                                // Show a toast message to help locate the file
                                Handler(Looper.getMainLooper()).post {
                                    Toast.makeText(
                                        serviceContext,
                                        "Saved frame to Pictures/ScreenStream/$filename",
                                        Toast.LENGTH_LONG
                                    ).show()
                                }
                                XLog.d(getLog("saveFrame", "Saved frame to Pictures/ScreenStream/$filename"))
                            }
                        } catch (e: Exception) {
                            XLog.e(getLog("saveFrame", "Failed to save frame: ${e.message}"), e)
                            // Show error in UI
                            Handler(Looper.getMainLooper()).post {
                                Toast.makeText(
                                    serviceContext,
                                    "Failed to save frame: ${e.message}",
                                    Toast.LENGTH_LONG
                                ).show()
                            }
                        }
